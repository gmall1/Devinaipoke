...
import { ArrowLeft, ChevronDown, ChevronUp, Edit2, Plus, Save, Trash2, X, Loader2, Bot, Layers, Shuffle } from "lucide-react";
        ...
        <div className="flex gap-2 border-b border-border pb-1">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-body transition-colors border-b-2 ${tab === t.id ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
              {t.icon}{t.label}
            </button>
          ))}
        </div>
