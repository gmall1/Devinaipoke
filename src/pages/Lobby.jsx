...
export default function Lobby() {
  ...
  const handleCreate = async (selectedMode) => {
    try {
      ...
      setError("");
      await persistName();
      const room = await createRoom(displayName.trim() || user?.full_name || "Local Player", selectedMode, buildStarterDeck());
  ...
  const handleJoin = async (code) => {
    try {
      ...
      setError("");
      await persistName();
      const room = await joinRoom(code.trim().toUpperCase(), displayName.trim() || user?.full_name || "Local Player", buildStarterDeck());
